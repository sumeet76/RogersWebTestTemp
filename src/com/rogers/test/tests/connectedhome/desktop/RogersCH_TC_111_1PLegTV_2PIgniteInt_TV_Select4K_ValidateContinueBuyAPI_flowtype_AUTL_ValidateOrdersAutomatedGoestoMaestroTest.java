 ...
 *11. Select 'yes' for   'Do you have a 4K TV'.
...
public class RogersCH_TC_111_1PLegTV_2PIgniteInt_TV_Select4K_ValidateContinueBuyAPI_flowtype_AUTL_ValidateOrdersAutomatedGoestoMaestroTest extends BaseTestClass {
	...
	public void checkTVPackageUpgradeTest() {
	    ...
	    getRogersAccountOverviewPage().selectAccount(TestDataHandler.tc111_Legacy1PtoIgnite2P.accountDetails.getBan());
        ...
        getRogersIgniteTVCreditCheckPage().selectDOBYearExistingCustomer(TestDataHandler.tc111_Legacy1PtoIgnite2P.getAccountDetails().getYear());
        getRogersIgniteTVCreditCheckPage().selectDOBMonthExistingCustomerMigration(TestDataHandler.tc111_Legacy1PtoIgnite2P.getAccountDetails().getMonth());
        getRogersIgniteTVCreditCheckPage().selectDOBDayExistingCustomerMigration(TestDataHandler.tc111_Legacy1PtoIgnite2P.getAccountDetails().getDate());
		...
		reporter.reportLogWithScreenshot("Clicked on No upgrade to professional upgrade radio button");
		...
		/*
		** DB validation from Subscriber Table
		 */
