...
public class RogersSolarisConsumer_TC_008_GWP_ExistingLegacyCustomer_InternetAndSHMCustomer_ServiceAndBrowserProvinceNBTest extends BaseTestClass {
	...
	public void checkGWPInternetAndSHMTest(String strLanguage) {
		...
		rogers_login_page.setUsernameIFrame(TestDataHandler.solarisConsumerGWPInternetSHM.getUsername());
		rogers_login_page.setPasswordIFrame(TestDataHandler.solarisConsumerGWPInternetSHM.getPassword());
		...
		if(strLanguage.equalsIgnoreCase("fr")) {
			rogers_igniteTV_buy_page.clkChevronForBundle(TestDataHandler.solarisConsumerGWPInternetSHM.getAccountDetails().getBundelPackageNameFr());
			Assert.assertTrue(rogers_igniteTV_buy_page.verifyGWPForStarterPackage(TestDataHandler.solarisConsumerGWPInternetSHM.getAccountDetails().getBundelPackageNameFr()), "GWP offer on starter package is not available");
			reporter.reportLogWithScreenshot("GWP promotion verified successfully in the cart");
			rogers_igniteTV_buy_page.selectSolarisBundlePackage(TestDataHandler.solarisConsumerGWPInternetSHM.getAccountDetails().getBundelPackageNameFr());
		}
		else {
			rogers_igniteTV_buy_page.clkChevronForBundle(TestDataHandler.solarisConsumerGWPInternetSHM.getAccountDetails().getBundelPackageNameEn());
			Assert.assertTrue(rogers_igniteTV_buy_page.verifyGWPForStarterPackage(TestDataHandler.solarisConsumerGWPInternetSHM.getAccountDetails().getBundelPackageNameEn()), "GWP offer on starter package is not available");
			reporter.reportLogWithScreenshot("GWP promotion verified successfully in the cart");
			rogers_igniteTV_buy_page.selectSolarisBundlePackage(TestDataHandler.solarisConsumerGWPInternetSHM.getAccountDetails().getBundelPackageNameEn());
		}
		...
		reporter.reportLogWithScreenshot("GWP promotion verified successfully in the cart");
		rogers_igniteTV_credit_check_page.clkChevronYourCart();
		rogers_igniteTV_credit_check_page.selectDOBYearExistingCustomer(TestDataHandler.solarisConsumerGWPInternetSHM.getAccountDetails().getYear());
		rogers_igniteTV_credit_check_page.selectDOBMonthExistingCustomer(TestDataHandler.solarisConsumerGWPInternetSHM.getAccountDetails().getMonth());
		rogers_igniteTV_credit_check_page.selectDOBDayExistingCustomer(TestDataHandler.solarisConsumerGWPInternetSHM.getAccountDetails().getDate());
